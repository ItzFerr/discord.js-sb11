...
class MessageCreateAction extends Action {
  handle(data) {
    try {
      ...
      if (channel) {
        ...
        if (data instanceof Array) {
          ...
          for (let i = 0; i < data.length; i++) {
            messages[i] = (typeof channel._cacheMessage === 'function')
              ? channel._cacheMessage(new Message(channel, data[i], client))
          ...
          const message = (typeof channel._cacheMessage === 'function')
            ? channel._cacheMessage(new Message(channel, data, client))
      ...
      console.error(`Error in MessageCreateAction: ${error.message}`);
    }
